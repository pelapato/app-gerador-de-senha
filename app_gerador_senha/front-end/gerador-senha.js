...
const senha_texto = document.querySelector('#campo-1>input[type=text]')
const input_range=document.querySelector('input[type=range]')
const num_tam = document.querySelector('#s-1>article>i')
const checkboxs = [...document.querySelectorAll('input[type=checkbox]')]
const barras = [...document.querySelectorAll('#s-3>article>strong>i')]
const texto_barras = document.querySelector('#s-3>article>p')
...
function gerar_senha(tamanho_caracteres){
    ...
    for(let x=0;x<=tamanho_caracteres;++x){
        ...
        if(opc_mi && opc_ma==false && opc_simb==false && opc_num==false){
            ...
            barras.map((e)=>{
                e.classList.add('fraca')
            })
            texto_barras.innerHTML="fraca"
        }
            ...
            barras.map((e)=>{
                e.classList.add('fraca-media')
            })
            texto_barras.innerHTML="media"
        }
            ...
            barras.map((e)=>{
                e.classList.add('forte-media')
            })
            texto_barras.innerHTML="forte"
        }
...
const copiar_aqui =async (texto)=>{
    try{
        const copiado = await navigator.clipboard.writeText(texto);
    }catch (e){
        console.log(e)
    }
}
function copiar(){
    const text = senha_texto.value == "" ? 'voce copiou nada' : senha_texto.value 
    copiar_aqui(text)
}